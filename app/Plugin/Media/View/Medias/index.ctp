...
jQuery(function(){
	...
	uploader.bind('FileUploaded', function(up, file, response){
		...
		if(response.error){
			...
			/* $('#pluploadPhoto').html('<img id="target" src="<?php echo Router::url('/img/'); ?>'+response.content+'" width="100%">').fadeIn(500);
     		 $('#pluploadPreview').html('<img src="<?php echo Router::url('/img/'); ?>'+response.content+'" id="preview">').fadeIn(500);*/
		}
		$('#'+file.id).remove();
	});
